...
var Typewriter = function Typewriter(srcElement) {
  ...
  var newElement = document.createElement(srcElement.nodeName);
  ...
  while (true) {
    ...
    if (travelNode.nodeName === 'SCRIPT') {
      // Skip scripts to avoid being called multiple times.
      self.processQueue.pop();
    } else if (travelNode.childNodes.length > 0 && !travelNode.matches(self.config.skip)) {
      self.processQueue.push({
        type: 'Operation',
        data: 'in'
      });
