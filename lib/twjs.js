...
var Typewriter = function Typewriter(srcElement, cfg) {
  ...
  while (true) {
    if (self.config.skipMathJax && travelNode.nodeType === 1 && travelNode.className.indexOf('MathJax') !== -1) {
      self.processQueue.push({
        type: 'Node',
        data: travelNode.cloneNode(true)
      });
      if (travelNode.className.indexOf('MathJax_Preview') === -1) {
        for (var i = 0; i < self.config.mathShock; i += 1) {
          self.processQueue.push({
            type: 'Wait',
            data: null
          });
    ...
    if (travelNode.childNodes.length > 0 && (!self.config.skipMathJax || travelNode.className.indexOf('MathJax') === -1)) {
      self.processQueue.push({
        type: 'Operation',
        data: 'in'
      });
